...
# 3 Summarise cover of benthos and score of complexity as point scores (TJL <- )
...
# Campaigns ----
# ID <- "2010-03_Perth.Canyon.60_stereoBRUVs" # BG 06/07/17
# ID <- "2010-03_Perth.Canyon.90_stereoBRUVs" # not analysed
# ID <- "2010-03_Perth.Canyon.180_stereoBRUVs"# not analysed
# ID <- "2010-11_Perth.Canyon.60_stereoBRUVs" # not analysed
# ID <- "2010-11_Perth.Canyon.90_stereoBRUVs" # not analysed
...
# Campaigns ----
# ID <- "2006-00_Capes.sanctuaries_stereoBRUVs"  # NAs in final data - Need to confirm with Tim if ok
# ID <- "2007-03_Capes.sanctuaries_stereoBRUVs" # NAs in final data - Need to confirm with Tim if ok
# ID <- "2008-02_Capes.sanctuaries_stereoBRUVs" # NAs in final data - Need to confirm with Tim if ok
# ID <- "2008-04_Capes.sanctuaries_stereoBRUVs" # not analysed
# ID <- "2009-03_Capes.sanctuaries_stereoBRUVs" # not analysed
# ID <- "2009-08_Capes.sanctuaries_stereoBRUVs" # not analysed
# ID <- "2010-01_Capes.sanctuaries_stereoBRUVs" # not analysed
# ID <- "2010-06_Capes.sanctuaries_stereoBRUVs" # not analysed

## Ningaloo Marine Park WAMSI_Fitzpatrick ----
# Project <- "Ningaloo Marine Park WAMSI_Fitzpatrick"

# Campaigns ----
# ID <- "2006-07_Ningaloo.sanctuaries_stereoBRUVs" # unable to find images/TMObs
# ID <- "2007-07_Ningaloo.sanctuaries_stereoBRUVs" # BG 07/07/17
...
# ID <- "2007-09_Rottnest.MF_stereoBRUVs" #07/07/17
...
## DOMGAS Pipeline Survey ----
# Project <- "DOMGAS Pipeline Survey"

# Campaigns ----
# ID <- "2010-10_Barrow.DOMGAS_stereoBRUVs" # Can't match OpCode's to Metadata - need to ask Curtin
# ID <- "2011-04_Barrow.DOMGAS_stereoBRUVs" 
...
habitat=paste(upload,"TMObs",sep="/")
...
# Department of Fisheries deepwater transects ----
#2010-03_Perth.Canyon.60_stereoBRUVs (Mistakes in OpCode) ----
# hab<-hab%>%
#   mutate(OpCode=paste("RC10",OpCode,"60",sep = "_"))# Adds prefix and suffix


# Nagari Capes long-term monitoring ----
# 2006-00_Capes.sanctuaries_stereoBRUVs (OpCode mistakes)----
# hab<-hab%>%
#   mutate(OpCode=substr(OpCode,1,5))%>%
#   mutate(Prefix=substr(OpCode,1,4))%>% # Next four lines fix errors in file names
#   mutate(Suffix=substr(OpCode,5,5))%>%
#   mutate(OpCode=paste(Prefix,Suffix,sep="-"))%>%
#   mutate(OpCode=paste("6",OpCode,sep=""))%>%
#   select(-c(Prefix,Suffix))

# 2007-03_Capes.sanctuaries_stereoBRUVs (OpCode mistakes)----
# hab<- hab%>%
#   filter(!OpCode%in%c("MW07-CNB7-1C71Andrew Tennyson"))%>%
#   mutate(OpCode= str_replace_all(.$OpCode,c("Alex Grochowski"="","Simon Grove"="","Di Watson"="","Kathy Zischka"="","Antony Payne"="","Ben Piek"="","Jordan Goetze"="","Cordelia Moore"="","Mike Byers"="","Maya Taing"="","Andrew Tennyson"="","Tim Langlois"="","Kirrily White"="","MW0"="", "C91"="","C71"="","C151"="","C131"="","C171"="","C271"="","C51"="","C241"="","C92"="","C31"="","C152"="","C72"="","C172"="","C52"="","C111"="")))%>%
#   filter(!OpCode%in%c("sDUNN2","sDUNN4","sQUIN1","sQUIN2","sQUIN3","sQUIN4","sQUIN5"))%>%
#   mutate(OpCode=str_replace(OpCode,"-",""))

# 2008-02_Capes.sanctuaries_stereoBRUVs ----

# Ningaloo Marine Park WAMSI_Fitzpatrick ----
# 2007-07_Ningaloo.sanctuaries_stereoBRUVs ----
# hab<-read.delim(data,header=T,skip=4,stringsAsFactors=FALSE)%>%
#   setNames(tolower(names(.)))%>%
#   mutate(OpCode=substr(filename,1,5))%>%
#   mutate(OpCode=str_replace(OpCode,"_",""))%>%
#   mutate(Prefix=substr(OpCode,1,3))%>% # Next four lines fix errors in file names
#   mutate(Suffix=substr(OpCode,4,6))%>%
#   mutate(Suffix=str_pad(Suffix,3,pad="0"))%>%
#   mutate(OpCode=paste("Ning07_",Prefix,Suffix,sep=""))%>%
# select(-c(filename,frame,time..mins.,date,location,site..,transect..,latitude,longitude,rugosity,depth,collector,fishing.status,spare,spare.1,code,x, x.1,x.2, x.3,radius..,Prefix,Suffix))%>%
...
# 2007-09_Rottnest.MF_stereoBRUVs(Mistakes in OpCode/filename)----
# hab <- hab%>%
#   mutate(OpCode= substr(OpCode,1,7))%>%
#   mutate(morphology=NA)%>%
#   mutate(type=NA)
...
head(hab)
unique(hab$OpCode)
sort(table(hab$OpCode),decreasing=TRUE) # To see how many times an OpCode is used - each should be 20
